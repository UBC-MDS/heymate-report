...
1. Data Ingestion: As mentioned in the [Introduction](#introduction), we used a Uber Eats dataset from Kaggle that covers diverse restaurant types, menu items, and descriptions.
...
1. **Data Cleaning**: To handle naming inconsistencies across restaurants, we used a GPT-powered semantic standardization pipeline to group similar items under unified labels (details in [Data Cleaning Module](#data-cleaning-module)).
...
Due to limited time, we decided to utilize GPT API as it can achieve semantic inference, format alignment, as well as language processing more efficiently while maintaining quality. We developed prompts that includes two main parts:
...
Originally, we planned to use Azure Functions following Developer Guide[@azurefunctions], aligning with our partner’s infrastructure, but security configuration issues prevented deployment. As a fallback, we deployed locally. A Looker Studio dashboard was also developed to monitor processing status and performance.
...
- **Semantic Filtering**: Support flexible, natural-language queries using embeddings or GPT-powered matching.
